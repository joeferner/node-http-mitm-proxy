...
CA.prototype.generateServerCertificateKeys = function (hosts, cb) {
  ...
  FS.writeFile(this.certsFolder + '/' + hostname.replace(/\*/g, '_') + '.pem', certPem);
  FS.writeFile(this.keysFolder + '/' + hostname.replace(/\*/g, '_') + '.key', keyPrivatePem);
  FS.writeFile(this.keysFolder + '/' + hostname.replace(/\*/g, '_') + '.public.key', keyPublicPem);
