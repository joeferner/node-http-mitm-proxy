...
Proxy.prototype._onHttpServerConnect = function(req, socket, head) {
  ...
  */
  if (head[0] == 0x16 || head[0] == 0x80 || head[0] == 0x00) {
    ...
    var hostname = Proxy.wilcard(req.url.split(':', 2)[0]);
    var sslServer = this.sslServers[hostname];
    if (sslServer && sslServer.port) {
      return makeConnection(sslServer.port);
    } else {
      if (!sslServer) {
    	  sslServer = this.sslServers[hostname] = { sem : semaphore(1) };
      }
      var sem = sslServer.sem;
      sem.take(function() {
        if (sslServer.port) {
        ...
        if (self.forceSNI) {
          if (self.httpsServer) {
        	...
        	self.httpsServer = createHttpsServer({}, function(httpsServer, wssServer) {
              ...
              if (!self.silent) {
                console.log('HTTPS server started on port %d', openPort);
              }
        	  self.httpsServer = httpsServer;
        	  self.wssServer = wssServer;
        	  self.httpsPort = openPort;
        	  if (!self.silent) {
                console.log('creating SNI context for ' + hostname);
              }
        	  self.httpsServer.addContext(hostname, results.httpsOptions);
          ...
          createHttpsServer(results.httpsOptions, function(httpsServer, wssServer) {
            ...
            self.sslServers[hostname].server = httpsServer;
            self.sslServers[hostname].wsServer = wssServer;
            return callback(null, openPort);
          });
        }
      });
...
Proxy.wilcard = function(hostname) {
  return hostname.replace(/^(.*)(\.[^\.]{4,}(\.[^\.]{1,3})*\.[^\.]+)$/, function(match, group1, group2) {
