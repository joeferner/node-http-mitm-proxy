...
Proxy.prototype.listen = function(options) {
  ...
  mkdirps(this.sslCaDir, function(err) {
    ...
    self.httpServer.on('clientError', self._onError.bind(self, "HTTP_CLIENT_ERROR", null));
...
Proxy.prototype._onHttpServerConnect = function(req, socket, head) {
  ...
  socket.on('error', self._onError.bind(self, "CLIENT_TO_PROXY_SOCKET_ERROR", null));
