...
Proxy.prototype.listen = function(options, callback) {
  ...
  if (this.http2 && !this.silent) {
    console.log('Experimental HTTP/2 support enabled');
  }
  this.httpAgent = options.httpAgent || new http.Agent();
  var HttpsAgent = (this.http2 ? http2 : https).Agent
  this.httpsAgent = options.httpsAgent || new HttpsAgent();
...
Proxy.prototype._createHttpsServer = function (options, callback) {
  var httpsServer = (this.http2 ? http2 : https).createServer(options);
...
Proxy.prototype._onHttpServerRequest = function(isSSL, clientToProxyRequest, proxyToClientResponse) {
  ...
  function makeProxyToServerRequest() {
    var proto = ctx.isSSL ? (self.http2 ? http2 : https) : http;
    ctx.proxyToServerRequestOptions.headers = self.filterAndCanonizeHeaders(ctx.isSSL, ctx.proxyToServerRequestOptions.headers);
  ...
  function proxyToServerRequestComplete(serverToProxyResponse) {
    ...
    return self._onResponse(ctx, function(err) {
      ...
      return self._onResponseHeaders(ctx, function (err) {
        ...
        ctx.proxyToClientResponse.writeHead(ctx.serverToProxyResponse.statusCode, self.filterAndCanonizeHeaders(ctx.isSSL, ctx.serverToProxyResponse.headers));
...
Proxy.prototype.filterAndCanonizeHeaders = function(isSSL, originalHeaders) {
  if (!originalHeaders) return originalHeaders;
  var headers = {};
  for (var key in originalHeaders) {
    var canonizedKey = key.trim();
    if (isSSL && this.http2 && ['connection', 'host', 'keep-alive', 'proxy-connection', 'transfer-encoding', 'upgrade'].indexOf(canonizedKey) !== -1) {
      // HTTP/2 deprecated header => filter
      continue;
    }
