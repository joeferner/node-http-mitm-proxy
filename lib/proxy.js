...
Proxy.prototype._onHttpServerConnect = function(req, socket, head) {
  ...
  */
  if (head[0] == 0x16 || head[0] == 0x80 || head[0] == 0x00) {
    ...
    var rootHost = Proxy.rootHost(hostname);
    var sem = self.sslSemaphores[rootHost];
    if (!sem) {
      sem = self.sslSemaphores[rootHost] = semaphore(1);
    }
    sem.take(function() {
      var serverHost = self._findMatchingSslServer(hostname);
      if (serverHost) {
        process.nextTick(sem.leave.bind(sem));
        if (serverHost !== hostname) {
          self.sslServers[hostname] = self.sslServers[serverHost];
        }
        return makeConnection(self.sslServers[hostname]);
      }
...
Proxy.prototype.onCertificateRequired = function (hostname, callback) {
  var self = this;
  return callback(null, {
    keyFile: self.sslCaDir + '/keys/' + hostname.replace(/\*/g, '_') + '.key',
    certFile: self.sslCaDir + '/certs/' + hostname.replace(/\*/g, '_') + '.pem'
  });
  return this;
};
Proxy.prototype.onCertificateMissing = function (ctx, files, callback) {
  this.ca.generateServerCertificateKeys(ctx.hostname, function (certPEM, privateKeyPEM) {
    callback(null, {
...
Proxy.HOSTNAME_REGEX = /^(.+)(\.[^\.]{4,}(\.[^\.]{1,3})*\.[^\.]+)$/;


/*
 * subdomain1.subdomain2.domain.com => [subdomain1.subdomain2.domain.com, *.subdomain2.domain.com, *.*.domain.com]
 * subdomain.domain.co.uk => [*.domain.co.uk, *.*.domain.co.uk]
 */
Proxy.prototype._findMatchingSslServer = function(hostname) {
  if (this.sslServers[hostname]) {
    return hostname;
  }
  var m = Proxy.HOSTNAME_REGEX.exec(hostname);
  if (m) {
    var subdomains = m[1].split('.');
    for (var i=0; i<subdomains.length; i++) {
      subdomains[i] = '*';
      var h = subdomains.join('.') + m[2];
      var sslServer = this.sslServers[h];
      if (this.sslServers[h]) {
        return h;
      }
    }
  }
...
Proxy.rootHost = function(hostname) {
  return hostname.replace(Proxy.HOSTNAME_REGEX, function(match, group1, group2) {
    return group2;
  });
};
