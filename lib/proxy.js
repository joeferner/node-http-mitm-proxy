...
Proxy.prototype.listen = function(options) {
  ...
  mkdirps(this.sslCaDir, function(err) {
    ...
    self.httpServer.on('error', self._onError.bind(self, null));
    self.httpServer.on('clientError', self._onError.bind(self, null));
...
Proxy.prototype._onHttpServerConnect = function(req, socket, head) {
  ...
  socket.on('error', self._onError.bind(self, null));
  ...
  function openHttpsServer(hostname, callback) {
    self.onCertificateRequired(hostname, function (err, files) {
      async.auto({
        ...
        httpsServer.on('error', self._onError.bind(self, null));
        httpsServer.on('clientError', self._onError.bind(self, null));
